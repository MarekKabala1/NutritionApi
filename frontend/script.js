...
const nutritionForm = document.getElementById('nutritionForm').addEventListener('submit', async function (event) {
	...
	try {
		const response = await fetch(`http://localhost:${port}/nutrition`, {
			method: 'POST',
			headers: {
...
document.getElementById('productForm').addEventListener('submit', async function (event) {
	...
	try {
		const response = await fetch(`http://localhost:${port}/nutrition/findOne/${productName}`, {
			method: 'GET',
			headers: {
				'Content-Type': 'application/json',
			},
